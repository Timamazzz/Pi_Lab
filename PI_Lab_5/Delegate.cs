...
namespace PI_Lab_5
{
    ...
    class Delegate
    {
        ...
        public double f1(double x) => Math.Pow(x,2)-2*Math.Cos(x)-1;

        public double f2(double x) => Math.Pow(x, 3) - Math.Pow(x, 2) + 2 * x - 4;

        public double df1(double x) => 2*x+2*Math.Sin(x);

        public double df2(double x) => 3 * Math.Pow(x, 2) - 2 * x + 2;
